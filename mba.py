...
st.markdown("<h6 style='text-align: left; color: black;'>Кому нужены ассоциативные правила</h6>", unsafe_allow_html=True)
st.write("""
	
	1. _Аналитикам_ для анализа рыночной корзины и построения рекомендаций по покупкам.
    
	2. _Врачам_ для анализа белковых последовательностей и медицинской диагностики.
    
    3. _Юристам_ для поиска различных закономерностей в судебных делах.
    
    4. _Для студентов других специальностей_ для общего понимания современных технологий в сфере анализа анализа данных.

	"""
)

st.markdown("<h2 style='text-align: left; color: black;'>Этапы разработки кейса</h2>", unsafe_allow_html=True)
st.image('mba-stages.png', width=700)
...
pipeline_bar.markdown(
    ...
    \n
    \n**1. Блок теории:**
    \nЗдесь студенту предлагается познакомиться с теоретической составляющей ассоциативных правил. Понять кому и когда данный метод может быть полезен и научиться самому строить ассоциативнык правила.
    \n**2. Расчетные задания на теорию:**
	\nЗдась студенту необходимо выполнить ряд расчетных заданий посвященных вычислению различных ассоциативных правил. 
    \n**3. Демонстрация метода:**
	\nВ этом блоке студенту демонстрируется применение ассоциативных правил при анализе рыночной корзины и поиска фильмов наиболее часто смотрящих вместе.
    \n**4. Оформление микросервиса Streamlit, выгрузка на сервер:**
